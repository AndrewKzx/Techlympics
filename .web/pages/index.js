...
import { Box, Button, Center, Container, Divider, HStack, Image, Input, Link, ListItem, Menu, MenuButton, MenuDivider, MenuItem, MenuList, Modal, ModalBody, ModalContent, ModalHeader, ModalOverlay, OrderedList, Spacer, Text, VStack } from "@chakra-ui/react"
...
export default function Component() {
  ...
  {state.show_loans.map((osknrpan, dzslvkcg) => (
  <ListItem key={dzslvkcg}>
  <HStack>
  <Box dangerouslySetInnerHTML={{"__html": osknrpan}}/>
  <Button onClick={(_e) => addEvents([Event("state.finish_item", {item:osknrpan})], (_e), {})} sx={{"height": "1.5em", "backgroundColor": "white", "textColor": "white", "fontSize": "1em"}}>
  {`❌`}
</Button>
</HStack>
</ListItem>
  ...
  <Box as={`form`} onSubmit={(_e0) => addEvents([Event("state.add_item", {form_data:{"interest": getRefValue(ref_interest), "loan": getRefValue(ref_loan), "installment": getRefValue(ref_installment), "sym": getRefValue(ref_sym), "name": getRefValue(ref_name)}})], (_e0), {"preventDefault": true})}>
  ...
  <Container>
  <Text sx={{"fontWeight": "bold", "fontSize": "1em", "color": "forestgreen"}}>
  {`Real Monthly Income vs Real Monthly Expenses`}
</Text>
  <Plot data={state.figure_plt_1}/>
  <Text>
  {state.figure_description_1}
</Text>
  <Text sx={{"fontWeight": "bold", "fontSize": "1em", "color": "forestgreen", "marginTop": "2rem"}}>
  {`Breakdown of Real Monthly Expenses (Stacked)`}
</Text>
  <Plot data={state.figure_plt_2}/>
  <Text>
  {state.figure_description_2}
</Text>
  <Text>
  {state.bard_ouput}
</Text>
