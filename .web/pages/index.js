...
export default function Component() {
  ...
  {state.show_loans.map((pwpejstk, i) => (
  <ListItem key={i}>
  <HStack>
  <Box dangerouslySetInnerHTML={{"__html": pwpejstk}}/>
  <Button onClick={(_e) => addEvents([Event("state.finish_item", {item:pwpejstk})], (_e))} sx={{"height": "1.5em", "backgroundColor": "white", "textColor": "white", "fontSize": "1em"}}>
  {`❌`}
</Button>
</HStack>
</ListItem>
  ...
  <Box as={`form`} onSubmit={(_e0) => addEvents([Event("state.add_item", {form_data:{"interest": getRefValue(ref_interest), "installment": getRefValue(ref_installment), "name": getRefValue(ref_name), "loan": getRefValue(ref_loan), "sym": getRefValue(ref_sym)}})], (_e0))}>
